...
def start_delivery():
    global car_speed, car_state2
    car_state2 = "go"
    car_speed = 40
    urlopen('http://' + ip + "/action?go=speed40")
    return jsonify({'message': 'Delivery started'})

def yolo_thread():
    global image_flag, thread_image_flag, frame, thread_frame, car_state2, car_speed
    while True:
        if image_flag == 1:
            thread_frame = frame
            ...
            if not detections.empty:
                ...
                for _, detection in detections.iterrows():
                    ...
                    if "stop" in label and conf > 0.75:
                        ...
                        time.sleep(1)
                        car_state2 = "go"
                        urlopen('http://' + ip + "/action?go=speed" + str(car_speed))
                        ...
                        time.sleep(3)
                        car_state2 = "go"
                        insert_or_update_database(car_speed, "복귀중")
                        urlopen('http://' + ip + "/action?go=speed" + str(car_speed))
